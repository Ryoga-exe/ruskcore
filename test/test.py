...
parser.add_argument(
    "-r", "--recursive", action="store_true", help="search file recursively"
)
parser.add_argument("-e", "--extension", default="hex", help="test file extension")
parser.add_argument(
    "-o", "--output_dir", default="results", help="result output directory"
)
parser.add_argument(
    "-t",
    "--time_limit",
    type=float,
    default=10,
    help="limit of execution time. set 0 to nolimit",
)
...
def test(file_name):
    result_file_path = os.path.join(
        args.output_dir, file_name.replace(os.sep, "_") + ".txt"
    )
...
if __name__ == "__main__":
    ...
    statusText = (
        "Test Result : " + str(sum(res_statuses)) + " / " + str(len(res_statuses))
    )
